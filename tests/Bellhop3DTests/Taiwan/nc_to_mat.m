...
for iday = 1 : size( files, 1 )
   ...
   ncid = netcdf.open( [ files( iday, : ) '.nc' ], 'NC_NOWRITE' );
   ...
   % plan view
   figure
   pcolor( Oceanography.lon, Oceanography.lat, squeeze( Oceanography.c( :, :, 15 ) )' ); shading flat; colorbar;
   %caxis( [ 1530 1550 ] )
   xlabel( 'Longitude (degrees)' )
   ylabel( 'Latitude (degrees)' )
   
   figure
   pcolor( Oceanography.x, Oceanography.y, squeeze( Oceanography.c( :, :, 5 ) )' ); shading flat; colorbar;
   %caxis( [ 1530 1550 ] )
   xlabel( 'x (km)' )
   ylabel( 'y (km)' )
   drawnow
   ...
   % side view
   lat_slice = 25;   % plot slice at this lattitude
   [ ~, jj ] = min( abs( Oceanography.lat - lat_slice ) );
   figure; pcolor( Oceanography.x, Oceanography.depth, squeeze( Oceanography.c( :, jj, : ) )' ); shading flat; colorbar;
   %caxis( [ 1530 1600 ] )
   xlabel( 'x (km)' )
   ylabel( 'Depth (m)' )
