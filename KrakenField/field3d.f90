PROGRAM FIELD3D
  ...
  INTEGER              :: iCorner1, isx, isy, isd1, ird1, itheta, NSets, iRec, IElementSource, INode, IdentifySourceElement
  REAL                 :: Rmin, Rmax, freq, atten = 0
  ...
  RcvrDepth: DO ird1 = 1, Pos%Nrd
     CALL ReadAllModeSets( MaxSet, MaxM, Pos%rd( ird1 ), NSets, M, k, phiR )   ! Get modes at the receiver depth   
        ...
        Source_y: DO isy = 1, Pos%Nsy
           ...
           Corner: DO ICorner1 = 1, 3 
              ...
              IF ( ModeFileName( INode )( 1 : 5 ) /= 'DUMMY' ) THEN
                 CALL GetModes( ModeFileName( INode ), 1, ifreq, MaxM, Pos%sd, Pos%Nsd, 'N', &
           ...
           SourceDepth: DO isd1 = 1, Pos%Nsd 
              ...
              IF ( Msource > 0 ) THEN
                 ...
                 CASE ( 'STD' )  ! STandDard (ignores hor. refraction)
                    CALL Evaluate3D(  freq, k, phiR, phiST, M, MaxM, IElementSource, Pos%sx( isx ), Pos%sy( isy ), &
                         Pos%theta, Pos%Ntheta, Rmin, Rmax, Pos%Nr, Mlimit, P )       
                 ...
                 CASE ( 'GBT' )  ! Gaussian Beam Tracing (include hor. refraction)
                    CALL EvaluateGB(  k, phiR, phiST, M, MaxM, IElementSource, Pos%sx( isx ), Pos%sy( isy ), &
                         Pos%theta, Pos%Ntheta, Rmin, Rmax, Pos%Nr, Mlimit, Option, P, freq )       
                 CASE DEFAULT
              ...
              IF ( ird1 == 1 .AND. isx == 1 .AND. isy == 1 .AND. isd1 == 1 ) THEN
                 CALL WriteHeader( SHDFileName, Title, atten, PlotType )
              END IF
              ...
              RcvrBearing: DO itheta = 1, Pos%Ntheta
                 iRec = 10 + ( isx    - 1 ) * Pos%Nsy * Pos%Ntheta * Pos%Nsd * Pos%Nrd + &
                             ...
                             ( isd1   - 1 ) *                        Pos%Nrd + ird1
                 WRITE( SHDFile, REC = iRec ) P( itheta, 1 : Pos%Nr )
              END DO RcvrBearing
...
SUBROUTINE READIN( FileRoot, Title, Option, Mlimit, Rmin, Rmax )
  ...
  IMPLICIT NONE
  LOGICAL, PARAMETER :: ThreeD = .TRUE.   ! used by read routines to signal to read from the file
  ...
  CALL Readsxsy(         FLPFile, PRTFile, ThreeD )      ! source x-y coordinates
  CALL ReadSdRd(         FLPFile, PRTFile, 0.0, 1.0E6 )  ! source/rcvr depths
  CALL ReadRcvrRanges(   FLPFile, PRTFile )
  CALL ReadRcvrBearings( FLPFile, PRTFile )              ! angles for radials
...
FUNCTION IdentifySourceElement( xs, ys ) 
  ...
  IF ( IdentifySourceElement == 0 ) CALL ERROUT( PRTFile, 'F', 'FIELD3D - IdentifySourceElement', 'Source not inside the grid' ) 
...
SUBROUTINE ReadAllModeSets( MaxSet, MaxM, rd, NSets, M, k, phiR )      
  ...
  NodeLoop: DO INode = 1, NNodes 
     ...
     IF ( NSets > MaxSet ) THEN 
        ...
        CALL ERROUT( PRTFile, 'F', 'FIELD3D - ReadAllModeSets', 'Too many mode sets' )               
     ENDIF
     Iset( INode ) = NSets 
     ...
     ELSE  ! Necessary to read in modes
        CALL GetModes( ModeFileName( INode ), 1, ifreq, MaxM, rd, 1, 'N', k( 1, NSets ), phiR( 1, NSets ), M( NSets ), &
