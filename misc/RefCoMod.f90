...
CONTAINS
  ...
  SUBROUTINE ReadReflectionCoefficient( FileRoot, BotRC, TopRC, PRTFile )
    ...
    IF ( BotRC == 'F' ) THEN
       ...
       IF ( IOStat /= 0 ) THEN
          ...
          CALL ERROUT( PRTFile, 'F', 'ReadReflectionCoefficient', 'Unable to open Bottom Reflection Coefficient file' )
       END IF
       ...
       IF ( IAllocStat /= 0 ) &
          CALL ERROUT( PRTFile, 'F', 'ReadReflectionCoefficient', 'Insufficient memory for bot. refl. coef.: reduce # points'  )
    ...
    IF ( TopRC == 'F' ) THEN
       ...
       IF ( IOStat /= 0 ) THEN
          ...
          CALL ERROUT( PRTFile, 'F', 'ReadReflectionCoefficient', 'Unable to open Top Reflection Coefficient file' )
       END IF
       ...
       IF ( iAllocStat /= 0 ) &
          CALL ERROUT( PRTFile, 'F', 'ReadReflectionCoefficient', 'Insufficient memory for top refl. coef.: reduce # points'  )
    ...
    IF ( BotRC == 'P' ) THEN
       ...
       IF ( IOStat /= 0 ) CALL ERROUT( PRTFile, 'F', 'ReadReflectionCoefficient', &
       ...
       IF ( iAllocStat /= 0 ) CALL ERROUT( PRTFile, 'F', 'ReadReflectionCoefficient', 'Too many points in reflection coefficient' )
  ...
  SUBROUTINE InterpolateReflectionCoefficient( RInt, R, NPts, PRTFile )
    ...
    IF     ( thetaIntr < R( iLeft  )%theta ) THEN
       ...
       CALL ERROUT( PRTFile, 'W', 'InterpolateReflectionCoefficient', 'Refl. Coef. being set to 0 outside tabulated domain' )
