...
Nsamples_per_wavelength = 30;   % finite difference sampling
...
for ifreq = 1 : nfreq
    ...
    for ir = 1 : nr - 1
        ...
        while range_march > Pos.r.range( ircvr_range )   % bracketted?
            
            deltar_temp = range_march - Pos.r.range( ircvr_range );  % range step to receiver
            ...
            if ( deltar_temp > 0 )
                ...
                psit( :, ircvr_range ) = interp1( z_tot, psitemp, Pos.r.depth, 'linear', 0 );
    ...
    % conugate the field to be consistent with the Acoustics Toolbox
    % convention
    is = 1;
