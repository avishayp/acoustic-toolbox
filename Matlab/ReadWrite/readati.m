...
if ( TopATI == '*' || TopATI == '~' )
    ...
    if ( strcmp( atifil, 'ATIFIL' ) == 0 && isempty( strfind( atifil, '.ati' ) ) )
    ...
    for ii = 1 : NatiPts
        ...
        if ( ii < 50 || ii == NatiPts )   % echo up to 51 values
            fprintf( '%9.5g    %9.5g \n', xTop( :, ii+1 ) );
        end
        if ( xTop( 2, ii ) < depthT )
    ...
    if ( atiType == 'C' ) % Curvilinear option: compute tangent and normal at node by averaging normals on adjacent segments
