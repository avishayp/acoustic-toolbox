...
disp( 'Should be a check in here that theta is montonically increasing' )
if ( TopRC == 'F' )
    
    disp( 'Reading top    reflection coefficient' )
    
    if ( strcmp( trcfil, 'TRCFIL' ) == 0 && isempty( strfind( trcfil, '.trc' ) ) )
...
if ( BotRC == 'F' )
    ...
    if ( strcmp( brcfil, 'BRCFIL' ) == 0 && isempty( strfind( brcfil, '.brc' ) ) )
