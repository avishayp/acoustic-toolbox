...
% dr = 1000 * ( Pos.r.range( end ) - Pos.r.range( 1 ) ) / length( Pos.r.range );   % range step
% dz =        ( Pos.r.depth( end ) - Pos.r.depth( 1 ) ) / length( Pos.r.range );   % depth step
...
ndr = round( ( Pos.r.range( 2 ) - Pos.r.range( 1 ) ) / dr );   % range skip factor
ndz = round( ( Pos.r.depth( 2 ) - Pos.r.depth( 1 ) ) / dz );   % depth skip factor

ndr = max( ndr, 1 );
ndz = max( ndz, 1 );

zmplt = Pos.r.depth( end );
...
fprintf( fid, '''%s'' ! Title \r\n', TitleEnv );
fprintf( fid, '%8.2f %8.2f %8.2f \t \t \t ! Frequency (Hz) zs zr \r\n', freq, Pos.s.depth( 1 ), Pos.r.depth( end ) );
fprintf( fid, '%8.2f %8.2f %3i   \t \t \t ! rmax dr ndr \r\n', 1000 * RMax, dr, ndr );
fprintf( fid, '%8.2f %8.2f %3i %8.2f   \t \t \t ! zmax dz ndz zmplt \r\n', zmax, dz, ndz, zmplt );
fprintf( fid, '%8.2f %1i %1i %8.2f   \t \t \t ! c0 np ns rs \r\n', c0, np, ns, rs );
...
for ii = 2 : NbtyPts - 1
   ...
   if ( ii ~= 2 )
      fprintf( fid, '\t %6.2f \t ! rp \r\n', xBot( 1, ii ) );   % range for this profile
   end
   ...
   % Sediment
   fprintf( fid, '    %6.2f %6.2f /  \t ! lower halfspace \r\n', 0.0, Bdry.Bot.HS.alphaR );
   fprintf( fid, '-1 -1 \r\n' );   % terminator
   ...
   % attenuation
   fprintf( fid, '    %6.2f %6.2f  \t ! lower halfspace \r\n', 0.0, Bdry.Bot.HS.alphaI );
   fprintf( fid, '    %6.2f %6.2f  \t ! lower halfspace \r\n', 1.9 * SSP.depth( SSP.NMedia+1 ), Bdry.Bot.HS.alphaI );
   fprintf( fid, '    %6.2f %6.2f  \t ! lower halfspace \r\n', 2.0 * SSP.depth( SSP.NMedia+1 ), 10 );
   fprintf( fid, '-1 -1 \r\n' );   % terminator
