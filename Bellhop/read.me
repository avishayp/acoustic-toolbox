...
A problem was also present in that BELLHOP3D did not always check for each new ray which batymetry/altimetry segment the source was over/under. This has been fixed; however, I need to review this since BELLHOP uses a different process where it *always* searches for the active segment. This is less efficient but safer. In any case, BELLHOP and BELLHOP3D should likely be set up the same way.
