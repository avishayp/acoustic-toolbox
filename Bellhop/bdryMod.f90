...
CONTAINS
  ...
  SUBROUTINE ReadATI( FileRoot, TopATI, DepthT, PRTFile )
    ...
    CASE ( '~', '*' )
       ...
       atiPt: DO ii = 2, NatiPts - 1
          ...
          CASE ( 'S', '' )
             ...
             END IF
          CASE ( 'L' )
             READ(  ATIFile, * )                   Top( ii )%x, Top( ii )%HS%alphaR, Top( ii )%HS%alphaI, Top( ii )%HS%rho, &
                                                                Top( ii )%HS%betaR,  Top( ii )%HS%betaI
             IF ( ii < Number_to_Echo .OR. ii == NatiPts ) THEN   ! echo some values
                WRITE( PRTFile, FMT = "(7G11.3)" ) Top( ii )%x, Top( ii )%HS%alphaR, Top( ii )%HS%alphaI, Top( ii )%HS%rho, &
                                                                Top( ii )%HS%betaR,  Top( ii )%HS%betaI
             END IF
          CASE DEFAULT
             CALL ERROUT( PRTFile, 'F', 'ReadATI', 'Unknown option for selecting altimetry option' )
          END SELECT
    ...
    IF ( .NOT. monotonic( Top%x( 1 ), NAtiPts ) ) THEN
       CALL ERROUT( PRTFile, 'F', 'BELLHOP:ReadATI', 'Altimetry ranges are not monotonically increasing' )
    END IF 
  ...
  SUBROUTINE ReadBTY( FileRoot, BotBTY, DepthB, PRTFile )
    ...
    CASE ( '~', '*' )
       ...
       IF ( IOsTAT /= 0 ) THEN
          ...
          CALL ERROUT( PRTFile, 'F', 'ReadBTY', 'Unable to open bathymetry file' )
       END IF
       ...
       btyPt: DO ii = 2, NbtyPts - 1
          ...
          CASE ( 'S', '' )   ! short format
             ...
             END IF
          CASE ( 'L' )       ! long format
             READ(  BTYFile, * )                   Bot( ii )%x, Bot( ii )%HS%alphaR, Bot( ii )%HS%alphaI, Bot( ii )%HS%rho, &
                                                                Bot( ii )%HS%betaR,  Bot( ii )%HS%betaI
             IF ( ii < Number_to_Echo .OR. ii == NbtyPts ) THEN   ! echo some values
                WRITE( PRTFile, FMT="( F10.2, F10.2, 3X, 2F10.2, 3X, F6.2, 3X, 2F10.4 )" ) &
                   Bot( ii )%x, Bot( ii )%HS%alphaR, Bot( ii )%HS%alphaI, Bot( ii )%HS%rho, &
                                Bot( ii )%HS%betaR,  Bot( ii )%HS%betaI
             END IF
          CASE DEFAULT
             CALL ERROUT( PRTFile, 'F', 'ReadBTY', 'Unknown option for selecting bathymetry option' )
          END SELECT
    ...
    IF ( .NOT. monotonic( Bot%x( 1 ), NBtyPts ) ) THEN
       CALL ERROUT( PRTFile, 'F', 'BELLHOP:ReadBTY', 'Bathymetry ranges are not monotonically increasing' )
    END IF 
  ...
  SUBROUTINE GetTopSeg( r )
    ...
    ELSE
       ...
       CALL ERROUT( PRTFile, 'F', 'GetTopSeg', 'Top altimetry undefined above the ray' )
    ENDIF
  ...
  SUBROUTINE GetBotSeg( r )
    ...
    ELSE
       ...
       CALL ERROUT( PRTFile, 'F', 'GetBotSeg', 'Bottom bathymetry undefined below the source' )
    ENDIF
