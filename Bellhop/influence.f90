SUBROUTINE InfluenceCervenyRayCen( U, epsilon, alpha, iBeamWindow2, RadMax )
  ...
  RcvrDepths: DO id = 1, Nrd_per_range
     ...
     Images: DO image = 1, Beam%Nimage
        ...
        Stepping: DO is = 2, Beam%Nsteps
           ...
           IF ( ABS( znV ) < tiny( znV ) ) CYCLE Stepping   ! If normal parallel to TL-line, skip to next step on ray
...
SUBROUTINE InfluenceCervenyCart( U, epsilon, alpha, iBeamWindow2, RadMax )
  ...
  REAL (KIND=8), INTENT( IN    ) :: alpha, RadMax                ! take-off angle
  COMPLEX,       INTENT( INOUT ) :: U( Nrd_per_range, Pos%Nr )   ! complex pressure field
  INTEGER          :: KMAHV( MaxN ), KMAH, is, id, ir, irA, irB
  REAL    (KIND=8) :: x( 2 ), rayt( 2 ), rayn( 2 ), Tr, Tz, rA, rB, zr, W, ratio1, &
                      c, cimag, cs, cn, csq, gradc( 2 ), crr, crz, czz, rho, Hermite, deltaz
  ...
  Stepping: DO is = 3, Beam%Nsteps
     ...
     irA = MAX( MIN( INT( ( rA - Pos%r( 1 ) ) / Pos%Delta_r ) + 1, Pos%Nr ), 1 ) ! should be ", 0 )" ?
     irB = MAX( MIN( INT( ( rB - Pos%r( 1 ) ) / Pos%Delta_r ) + 1, Pos%Nr ), 1 )

     IF ( irA >= irB ) CYCLE Stepping

     Ranges: DO ir = irA + 1, irB
...
SUBROUTINE InfluenceGeoHatRayCen( U, alpha, dalpha )
  ...
  USE ArrMod
  IMPLICIT NONE
  REAL (KIND=8), INTENT( IN    ) :: alpha                        ! take-off angle
  COMPLEX,       INTENT( INOUT ) :: U( Nrd_per_range, Pos%Nr )   ! complex pressure field
  INTEGER          :: is, id, ir, irA, irB, II
  REAL    (KIND=8) :: nA, nB, rA, rB, W, zr, znv, rnv, ratio1, n, Amp, phase, const, phaseInt, &
       q0, q, qold, SrcAngle, RcvrAngle, dalpha, L, dq( Beam%Nsteps - 1 )
  ...
  RcvrDepths: DO id = 1, Nrd_per_range
     ...
     znV = -ray2D( 1 )%t( 1 ) * ray2D( 1 )%c
     rnV =  ray2D( 1 )%t( 2 ) * ray2D( 1 )%c

     nA  = ( zR - ray2D( 1 )%x( 2 )   ) / znV
     rA  = ray2D( 1 )%x( 1 ) + nA * rnV

     !!! following assumes uniform spacing in Pos%r
     irA = MAX( MIN( INT( ( rA - Pos%r( 1 ) ) / Pos%Delta_r ) + 1, Pos%Nr ), 1 ) ! index of receiver
     ...
     Stepping: DO is = 2, Beam%Nsteps
        ...
        !!! following assumes uniform spacing in Pos%r
        irB = MAX( MIN( INT( ( rB - Pos%r( 1 ) ) / Pos%Delta_r ) + 1, Pos%Nr ), 1 ) ! index of receiver
        ...
        Ranges: DO ir = irA + 1 - II, irB + II, SIGN( 1, irB - irA )  ! Compute influence for each rcvr
           W = ( Pos%r( ir ) - rA ) / ( rB - rA )
           n = ABS( nA                + W * ( nB - nA ) )
           ...
           IF ( n < L ) THEN   ! in beamwindow?
              ...
              CASE DEFAULT      ! incoherent/semi-coherent TL
                 U( id, ir ) = U( id, ir ) + SNGL( ( Amp * EXP( AIMAG( omega * delay ) ) / W ) ** 2 * W )
              END SELECT
           END IF
        END DO Ranges
        rA  = rB
...
SUBROUTINE InfluenceGeoHatCart( U, alpha, Dalpha )
  ...
  ! Computes the beam influence, i.e. the contribution of a single beam to the complex pressure
  ! This version uses geometric, hat-shaped beams in Cartesisan coordinates
  ...
  REAL    (KIND=8) :: x_ray( 2 ), rayt( 2 ), rayn( 2 ), x_rcvr( 2 ), s, n, phaseInt, &
                      rA, rB, rLen, RadMax, zMin, zMax, q0, qold, const, ratio1, W, &
           ...
           RcvrDepths: DO id = 1, Nrd_per_range
              ...
              IF ( n < RadMax ) THEN
                 ...
                 CASE ( 'C'  )     ! coherent TL
                    U( id, ir ) = U( id, ir ) + CMPLX( Amp * EXP( -i * ( omega * delay - ray2D( is - 1 )%Phase - phaseInt ) ) )
                    !U( id, ir ) = U( id, ir ) + CMPLX( Amp * EXP( -i * ( omega * delay - ray2D( is - 1 )%Phase - phaseInt + &
                         !omega * n * n / ( 2 * ray2d( is )%c**2 * delay ) ) ) )
...
SUBROUTINE InfluenceGeoGaussianCart( U, alpha, Dalpha )
  ...
  ! Computes the beam influence, i.e. the contribution of a single beam to the complex pressure
  ! This version uses geometric, Gaussian beams in Cartesian coordintes
  ...
  INTEGER          :: is, id, ir, irT( 1 ), irTT
  REAL    (KIND=8) :: x_ray( 2 ), rayt( 2 ), rayn( 2 ), x_rcvr( 2 ), s, n, phaseInt, &
                      rA, rB, rLen, RadMax, zMin, zMax, q, q0, qold, sigma, lambda, A, const, ratio1, W, &
                      Amp, phase, dqds, SrcAngle, RcvrAngle
  COMPLEX (KIND=8) :: delay, dtauds
  ...
  Stepping: DO is = 2, Beam%Nsteps
     ...
     ! phase shifts at caustics
     q  = ray2D( is - 1 )%q( 1 )
     IF ( q <= 0.0 .AND. qOld > 0.0 .OR. &
          q >= 0.0 .AND. qOld < 0.0 ) phase = phase + pi / 2.    ! phase shifts at caustics
     qold = q
     ...
     !sigma  = MAX( sigma, MIN( 0.2 * is * Beam%deltas, pi * lambda ) )
     ...
     RcvrRanges: DO
        ...
        RcvrDepths: DO id = 1, Nrd_per_range
           ...
           sigma  = MAX( sigma, MIN( 0.2 * freq * REAL( ray2D( is )%tau ), pi * lambda ) )
           !sigma  = MAX( sigma, MIN( 0.2 * is * Beam%deltas, pi * lambda ) )
           !write( PRTFile, * ) is, abs( q/q0),  0.2 * freq * REAL( ray2D( is )%tau ), .5 * lambda, sigma
           ...
           IF ( n < BeamWindow * sigma ) THEN   ! Within beam window?
              ...
              CASE ( 'A', 'a' )           ! arrivals
                 RcvrAngle  = RadDeg * ATAN2( rayt( 2 ), rayt( 1 ) )
                 CALL AddArr( omega, id, ir, Amp, ray2D( is )%Phase + phaseInt, delay, &
                      SrcAngle, RcvrAngle, ray2D( is )%NumTopBnc, ray2D( is )%NumBotBnc )
              CASE ( 'C' )                ! coherent TL
                 U( id, ir ) = U( id, ir ) + CMPLX( Amp * EXP( -i * ( omega * delay - ray2D( is )%Phase - phaseInt ) ) )
              CASE DEFAULT                ! incoherent/semicoherent TL
                 U( id, ir ) = U( id, ir ) + SNGL( SQRT( 2. * pi ) * ( const * EXP( AIMAG( omega * delay ) ) ) ** 2 * W )
              END SELECT
           END IF
        END DO RcvrDepths
        END IF
