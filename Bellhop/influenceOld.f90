SUBROUTINE InfluenceR( U, epsilon, alpha, iBeamWindow2, RadMax )
  ...
  RcvrDepths: DO id = 1, Nrd_per_range
     ...
     Images: DO image = 1, Beam%Nimage
        ...
        !!! If fixed this in InfluenceGeoHatRayCen
        ir1 = HUGE( ir1 )
...
SUBROUTINE InfluenceGeoHatRayCen( U, alpha, dalpha )
  ...
  ! Computes the beam influence, i.e. the contribution of a single beam to the complex pressure
  ! This version uses geometric, hat-shaped beams in ray-centered coordinates
  ...
  COMPLEX,       INTENT( INOUT ) :: U( Nrd_per_range, Pos%Nr )   ! complex pressure field
  INTEGER          :: is, id, ir, ir1, ir2, image
  ...
  RcvrDepths: DO id = 1, Nrd_per_range
     ...
     Images: DO image = 1, 1 !!! Beam%Nimage
        phase = 0.0
        qOld  = ray2D( 1 )%q( 1 )       ! used to track KMAH index

        !!! what about logic for different images?
        ...
        !!! following assumes uniform space in Pos%r
        ir1 = MAX( MIN( INT( ( rA - Pos%r( 1 ) ) / Pos%Delta_r ) + 1, Pos%Nr ), 1 ) ! index of receiver
        ...
        Stepping: DO is = 2, Beam%Nsteps
           ...
           Ranges: DO ir = ir1 + 1, ir2    ! Compute influence for each rcvr
              ...
              IF ( n < L ) THEN   ! in beamwindow?
                 ...
                 Amp   = const * ( L - n ) / L   ! hat-shaped beam
